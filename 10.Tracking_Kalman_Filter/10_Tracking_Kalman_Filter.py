...
X_Position_Kalman_Saved = np.zeros(n_samples)
Y_Position_Kalman_Saved = np.zeros(n_samples)
...
for i in range(n_samples):
    ...
    x_kalman, P, kalman_gain = Kalman_Filter(X, P, A, H, Q, R, measured, x_kalman, i)

    X_Position_Measured_Saved[i] = measured[0]
    Y_Position_Measured_Saved[i] = measured[1]
    X_Position_Kalman_Saved[i] = x_kalman[0]
    Y_Position_Kalman_Saved[i] = x_kalman[2]

fig = plt.figure(figsize=(8, 8))
plt.gca().invert_yaxis()
plt.scatter(X_Position_Measured_Saved, Y_Position_Measured_Saved, s=150, c="b", marker='*', label='Position: Measurements')
plt.scatter(X_Position_Kalman_Saved, Y_Position_Kalman_Saved, s=60, c="r", marker='o', label='Position: Kalman Filter')
plt.legend(loc='upper left')
plt.title('Position : Measurement vs Estimation(KF)')
...
#     plt.scatter(X_Position_Kalman_Saved[i], Y_Position_Kalman_Saved[i], s=60, c="r", marker='o', label='Position : Kalman Filter')
#     plt.legend(loc='upper left')
#     plt.title('Position: True vs Measurement vs Estimation(KF)')
#     plt.xlabel('X-Position[m]')
#     plt.ylabel('Y-Position[m]')
#     plt.xlim((-10, 350))
#     plt.ylim((250, -10))
